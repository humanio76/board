...
//		String sql = "select bno, title, name, regdate,"
//				+ " readcount,re_lev from board where bno>0 order by re_ref desc, re_seq asc";
	...
	//조회수 업데이트
	// update 테이블명 set readcount=readcount+1(바뀔내용) where bno=?
	...
	//내용 가져오기
	// select bno,name,title,content,attach(필드명) from board where bno=?
	...
	//비밀번호 확인
	//수정,삭제 시 확인
	// select * from board set password where bno=? and password=?
	...
	//댓글
	public int replyAricle(BoardVO vo) {
		int result=0;
		Connection con=null;
		PreparedStatement pstmt=null;
		//기존 댓글 re_seq 값 변경하기		
		...
		try {
			...
			sql="insert into board(bno,name,password,title,content,attach,re_seq,re_ref,re_lev) "
...
//		String sql = "select bno,title,name,regdate,readcount,re_lev"
//				+" from board where "+ criteria +" like ? and bno>0 order by "
